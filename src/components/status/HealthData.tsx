...
import type { HealthRecord, HealthDataResponse } from "@/lib/status-api";
import { fetchHealthData } from "@/lib/status-api";
  ...
  if (loading && !data) {
    return (
      <div className="flex items-center justify-center py-8">
        <p className="text-xs text-(--status-text-muted)">加载健康数据中...</p>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="text-center py-8">
        <p className="text-xs text-(--status-text-muted)">健康数据加载失败</p>
      </div>
    );
  }
  ...
  if (!data || data.records.length === 0) {
    return (
      <div className="text-center py-8">
        ...
        <p className="text-xs text-(--status-text-muted)">今天还没有健康数据呢~</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-2">
          {coreTypes.map((type) => {
            ...
            return (
              <div
                key={type}
                className="border border-dashed border-(--status-border) rounded-md p-3"
              >
                ...
                <div className="flex items-baseline gap-1">
                  <span className="text-lg font-mono font-semibold text-(--status-text)">
      ...
      {heartRatePoints.length >= 2 && (
        <div className="border border-dashed border-(--status-border) rounded-md p-3">
          <p className="text-[10px] text-(--status-text-muted) mb-2">今日心率趋势</p>
      ...
      {secondaryTypes.length > 0 && (
        <div className="border border-dashed border-(--status-border) rounded-md p-2">
              ...
              return (
                <div key={type} className="flex items-center justify-between px-2 py-1">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-xs text-(--status-text-muted)">
                      {meta?.label ?? type}
                    </span>
                  </div>
                  <div className="flex items-baseline gap-1">
                    <span className="text-xs font-mono font-medium text-(--status-text)">
                      {formatValue(entry.latest.value, type)}
                    </span>
                    <span className="text-[10px] text-(--status-text-muted)">
                      {entry.latest.unit}
...
function HeartRateChart({ points }: { points: { time: Date; value: number }[] }) {
  ...
  return (
    ...
    >
      {/* Grid lines */}
      <line x1={padX} y1={padY} x2={padX} y2={height} stroke="var(--status-border)" strokeWidth="0.5" />
      <line x1={padX} y1={height} x2={width - padX} y2={height} stroke="var(--status-border)" strokeWidth="0.5" />
      ...
      {/* Min/max labels */}
      <text x={padX - 3} y={padY + 6} textAnchor="end" fontSize="10" fill="var(--status-text-muted)" fontFamily="JetBrains Mono, monospace">
        {Math.round(maxVal)}
      </text>
      <text x={padX - 3} y={height} textAnchor="end" fontSize="10" fill="var(--status-text-muted)" fontFamily="JetBrains Mono, monospace">
        {Math.round(minVal)}
      </text>
    </svg>
  );
